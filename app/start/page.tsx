...
export default function Start() {
  ...
  async function createAccount() {
    ...
    try { setAccount(await post("/api/signup", { email: email.trim(), sellerUrl: profile.domain })); }
    catch (e) { setError(String(e instanceof Error ? e.message : e)); }
    finally { setBusy(null); }
  }
          ...
          <div className="stepcard">
            ...
            <div className="demo-input">
              ...
              {!profile && <button onClick={buildProfile} disabled={busy === "profile" || !sellerUrl.trim()}>
                {busy === "profile" ? "Reading your site…" : "Build my profile"}
              </button>}
            </div>
            {profile && (
            ...
            <div className="stepcard">
              ...
              <div className="demo-input">
                ...
                <button onClick={firstKit} disabled={busy === "kit" || !prospectUrl.trim()}>
                  {busy === "kit" ? "Generating (~60s)…" : "Generate"}
                </button>
              </div>
              {kit && (
